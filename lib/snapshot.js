...
export class ShadowSnapshotEngine {
  ...
  _resolveCwd(customCwd) {
    const raw = (customCwd && typeof customCwd === "object") ? customCwd.cwd : customCwd;
    return raw || this.cwd || process.cwd();
  }
  ...
  async cleanupOrphanedIndices(targetCwd) {
    const cwd = this._resolveCwd(targetCwd);
    if (!(await this._hasGit(cwd))) return;
    try {
  ...
  createSnapshot(label, { sessionId = '', cwd: customCwd, skipIfNoChanges = false } = {}) {
    return this._enqueue(() => this._doCreateSnapshot(label, { sessionId, cwd: customCwd, skipIfNoChanges }));
  }

  async _doCreateSnapshot(label, { sessionId = '', cwd: customCwd, skipIfNoChanges = false } = {}) {
    const cwd = this._resolveCwd(customCwd);
    const id = `snap-${this._now()}-${crypto.randomBytes(3).toString('hex')}`;
    const cleanLabel = String(label || 'checkpoint').replace(/[\r\n]+/g, ' ').trim().slice(0, 100);
    ...
    if (await this._hasGit(cwd)) {
      if (skipIfNoChanges) {
        ...
        if (lastSnap && lastSnap.commit) {
          try {
            const { stdout: statusOut } = await this.exec('git', ['status', '--porcelain'], { cwd });
            if (!statusOut.trim()) {
              const { stdout: headOut } = await this.exec('git', ['rev-parse', '--verify', 'HEAD'], { cwd }).catch(() => ({ stdout: '' }));
              if (headOut.trim() === lastSnap.commit) {
                return lastSnap;
              }
            }
          } catch {
            // fallback to full snapshot creation if fast status check fails
          }
        }
      }
      ...
      try {
        try {
          const stagedRes = await this.exec('git', ['write-tree'], { cwd });
          stagedTreeHash = stagedRes.stdout.trim() || null;
        } catch (err) {
          // best-effort staged tree capture; index might be dirty or conflicted
          console.warn('[dsh-time-machine] git write-tree failed during snapshot creation:', err.message || err);
        }
  ...
  rollbackSnapshot(id, { confirm = false, cwd: customCwd, restoreStaging = false } = {}) {
    return this._enqueue(() => this._doRollbackSnapshot(id, { confirm, cwd: customCwd, restoreStaging }));
  }

  async _doRollbackSnapshot(id, { confirm = false, cwd: customCwd, restoreStaging = false } = {}) {
    ...
    const cwd = this._resolveCwd(customCwd);
    if (!snap.commit) {
      throw new Error(`Snapshot ${id} does not have an associated git commit for rollback`);
    }
    if (!(await this._hasGit(cwd))) {
      throw new Error('Workspace is not a git repository for rollback');
    }
    if (snap.commit && await this._hasGit(cwd)) {
      try {
        ...
        if (restoreStaging && snap.stagedTreeHash) {
          try {
            await this.exec('git', ['read-tree', snap.stagedTreeHash], { cwd });
          } catch (err) {
            console.error('[dsh-time-machine] failed to restore staged tree:', err.message || err);
            throw new Error('Failed to restore staging area: ' + (err.message || err));
          }
        }
      } catch (e) {
  ...
  rollbackFile(id, filePath, { confirm = false, cwd: customCwd } = {}) {
    return this._enqueue(() => this._doRollbackFile(id, filePath, { confirm, cwd: customCwd }));
  }

  async _doRollbackFile(id, filePath, { confirm = false, cwd: customCwd } = {}) {
    if (!confirm) throw new Error('confirm required for rollback');
    if (!filePath || typeof filePath !== 'string') throw new Error('filePath required for selective rollback');
    const rawPath = filePath.trim();
    if (!rawPath) throw new Error('Invalid filePath');
    if (path.isAbsolute(rawPath)) {
      throw new Error('Absolute paths are not allowed for selective rollback');
    }
    ...
    if (rel === '' || rel.startsWith('..') || path.isAbsolute(rel)) {
      throw new Error('Path traversal detected: filePath must be inside workspace');
    }

    // Check symlink escapes: resolvedPath and its existing ancestors must be inside realCwd
    const realCwd = await fs.realpath(cwd).catch(() => path.resolve(cwd));
    try {
      const realTarget = await fs.realpath(resolvedPath);
      const relReal = path.relative(realCwd, realTarget);
      if (relReal.startsWith('..') || path.isAbsolute(relReal)) {
        throw new Error('Symlink traversal detected: resolved path is outside workspace');
      }
    } catch (err) {
      ...
      while (dir && dir !== path.dirname(dir)) {
        try {
          ...
          const relDir = path.relative(realCwd, realDir);
          if (relDir.startsWith('..') || path.isAbsolute(relDir)) {
            throw new Error('Symlink traversal detected: parent directory is outside workspace');
          }
          break;
        } catch (e) {
          if (e.code === 'ENOENT') {
    ...
    const cleanPath = rel.replace(/\\/g, '/');

    if (!snap.commit) {
      throw new Error(`Snapshot ${id} does not have an associated git commit for file rollback`);
    }
    if (!(await this._hasGit(cwd))) {
      throw new Error('Workspace is not a git repository for file rollback');
    }
    if (snap.commit && await this._hasGit(cwd)) {
      try {
        const checkRes = await this.exec('git', ['ls-tree', snap.commit, cleanPath], { cwd }).catch(() => ({ stdout: '' }));
        if (checkRes.stdout.trim()) {
          await this.exec('git', ['checkout', snap.commit, '--', cleanPath], { cwd });
        } else {
        ...
        console.warn('[dsh-time-machine] file rollback error:', e.message || e);
        throw new Error('Selective rollback failed: ' + (e.message || e));
      }
    }
    return { rolledBack: true, id, file: cleanPath };
  }
  ...
  async diff(fromId, toId = undefined, { cwd: customCwd, format = 'patch' } = {}) {
    ...
    if (!(await this._hasGit(cwd)) || !snapFrom.commit) {
      ...
      return { files: [], from: fromId, to: toId || "HEAD", diff: `diff ${snapFrom.label} -> ${toLabel} (no git repo)`, format };
    }
    ...
    try {
      ...
      if (diffOutput.length > MAX_DIFF_LENGTH) {
        ...
        truncated = true;
            ...
            let files = [];
      try {
        const numstatArgs = ['diff', '--numstat', fromRef];
        if (snapTo) numstatArgs.push(snapTo.commit || snapTo.ref);
        const { stdout: numstatOut } = await this.exec('git', numstatArgs, { cwd });
        files = numstatOut.trim().split('\n').filter(Boolean).map(line => {
          const [adds, dels, fpath] = line.split('\t');
          return {
            path: fpath,
        ...
        // best-effort numstat parsing; fallback to raw diff
        console.warn('[dsh-time-machine] diff numstat parsing error:', err.message || err);
      }

      return { diff: diffOutput, files, from: fromId, to: toId || 'HEAD', format, truncated };
    } catch (e) {
      console.warn('[dsh-time-machine] diff computation error:', e.message || e);
      return { files: [], diff: `[diff error: ${e.message || e}]`, from: fromId, to: toId || 'HEAD', format };
    }
  }
    ...
    for (const line of refs) {
      ...
      if (parts.length >= 4 && parts[2] && parts[3] !== undefined) {
        ...
        try {
          ...
          // best-effort git log info extraction for ref metadata
        }
      }
  ...
  setMax(maxSnapshots, targetCwd) {
    this.maxSnapshots = Math.max(1, Number(maxSnapshots) || 20);
    const sessions = [...new Set(this.snapshots.map(s => s.sessionId))];
    return this._enqueue(async () => {
      for (const sid of sessions) await this._trimFor(sid, targetCwd);
      if (sessions.length === 0) await this._trimFor('', targetCwd);
    }).catch((error) => {
      console.warn('[dsh-time-machine] setMax trim failed:', error?.message || error);
    });
  }
}